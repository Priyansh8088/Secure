...
void des_encrypt(uint64_t plain, uint64_t *cipher, uint8_t keys[16][48]) {
    uint8_t plain_bits[64], ip[64], L[32], R[32], temp[32];
    for (int i = 0; i < 64; i++)
        plain_bits[63 - i] = (plain >> i) & 1;
    permute(ip, plain_bits, IP, 64);
    memcpy(L, ip, 32);
    memcpy(R, ip + 32, 32);
    for (int round = 0; round < 16; round++) {
        memcpy(temp, R, 32);
...
int main() {
    ...
    for (int i = 0; i < blocks; i++) {
        ...
        for (int j = 0; j < 8; j++) {
            int idx = i * 8 + j;
            uint8_t c = (idx < len) ? buffer[idx] : 0;
            plain_block |= ((uint64_t)c) << ((7 - j) * 8);
        }
        uint64_t cipher_block;
        des_encrypt(plain_block, &cipher_block, keys);
        char hex[17];
        sprintf(hex, "%016llX", cipher_block);
        strcat(cipher_text, hex);
    }

    send(sock, cipher_text, strlen(cipher_text), 0);
